...
| [`images/rk3588/`](images/rk3588/) | The `rk3588` variant's own overlay: `build_files/` (installs the aic8800 driver rpm from `images/deps/`, `mesa-libTeflon`) -- see [`images/rk3588/README.md`](images/rk3588/README.md) |
...
| `.github/workflows/build-deps.yml` | Builds and pushes `images/deps/`'s side image -- only on an `images/deps/` push (its own `versions.env` included), or its own biweekly schedule (one day ahead of `build.yml`'s), never on every main-image build. |
