...
| `.github/workflows/build-flash.yml` | Called as a reusable workflow from a successful `build.yml` run (`uses:`, not a separate triggered run) -- matrixes over every board to build the raw disk artifact, then a single job publishes a release (tagged `v<fedora-version>.<date>.<N>`, auto-incrementing per day) once every board's build has finished (not once per board, to avoid races on a shared release). A direct `workflow_dispatch` builds the flash images without publishing a release. |
