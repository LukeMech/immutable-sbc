...
#
# Migrated verbatim from images/rpi/build_files/10-hailo8-pcie-driver.sh -- only
...
# attached. The userspace side (HailoRT, npu-run's Hailo backend) is built in
# 30-hailort-hailo8.sh instead: Hailo has no standard TFLite delegate (confirmed --
# there's an open upstream request for one, still unresolved), so npu-run drives it
# through HailoRT's own Python API instead of --delegate.
