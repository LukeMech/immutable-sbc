...
#
# Migrated verbatim from images/rk3588/build_files/10-aic8800-wifi-bt.sh -- only
# change is the final `cp` to /rpms/kmods/ instead of the main image installing this
# rpm directly. See that file's own git history for the full original story
# (radxa-pkg/aic8800 vs. the old ausil/aic8800-dkms COPR, the quilt patch series,
# the two-destination firmware install) -- kept here rather than re-explained.
#
...
# approach as 11-gasket.sh, no DKMS involved even though upstream's own Debian
# packaging wraps this in one (their debian/*.dkms files exist only for their own
# .deb; nothing here goes through the dkms tool).
#
